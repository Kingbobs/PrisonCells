...
class Cell{
	...
	public function updateSign(){
		if($this->owner != null and $this->owner !== ""){
			// RENTED
			$ownerName = strtolower($this->owner);
			if($p = $this->prisonCells->getServer()->getPlayerExact($ownerName)) $ownerName = $p->getName();
			...
			if($this->sign->namedtag instanceof CompoundTag){
				...
				$this->sign->namedtag->Text4 = new StringTag("Text4", $texts[3]);
	...
	public function isInside(Position $pos){
		if($pos->getLevel()->getFolderName() !== $this->lvl->getFolderName()) return false;
	...
	public function getExpireString(){
		return PrisonCells::timeToString($this->expireTime - time());
	}
